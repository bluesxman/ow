...
2. For each ability in `affected[i].abilities`, find matching bullets in `.run/patch-notes.md` under the `#### <Hero>` → `- **<Ability>**` subtree. Apply **quantitative** changes only to the matching `hero.abilities[]` entry (look up by `name`):
...
5. **Do not edit** any entry listed under `affected[i].skipped_abilities` — those are patch mentions for abilities the hero doesn't currently carry in `hero.abilities[]` (e.g., perks we don't track numerically, cross-referenced Stadium powers). Note them in the PR body instead.
...
- The matching `hero.abilities[i].*` numeric/string field for the natural-language → field mappings listed above.
...
- **Perk numeric tweaks.** Perks in `hero.perks.*` carry only `name` and `description`, not raw numbers. Per-perk cost lines and per-perk numeric tweaks are skipped.
